...
class PnP_ADMM(nn.Module):
    ...
    def forward(self, kernel, initial_uv, f, clean, sigma=25.5, lamb=690, sigma2=1.0, denoisor_sigma=25, irl1_iter_num=10, eps=1e-5,alpha = 0.): 
        # init
        f *= 255
        u  = f
        v  = f
        ...
        for k in range(self.nb):
            ...
            # This inner iteration solves the subproblem: argmin_v lambda <1, Kv-f logKv> + rho/2 \|v - (b+u) \|^2
            lamb_ = lamb
            # bx = 0*x
            ...
            for iii in range(10):
                ...
                fenzi2 = b + u + rho* torch.real(deblur.ifftn(fft_kH*deblur.fftn(x+bx)))
...
def plot_psnr(denoisor_level, lamb, sigma,alpha):
    device = 'cuda'
    model = PnP_ADMM()
    model.to(device)
    model.net.to(device)
    ...
    with torch.no_grad():
        ...
        model(kernel, initial_uv, img_L, img_H, sigma, lamb, sigma2, denoisor_level, 5, 1e-5,alpha)
    savepth = 'images/'
...
def search_args():
    ...
    # kernel 1
    # search_range[0.15] = [600] # 26.6561, 0.7270
    # kernel 2
    # search_range[0.15] = [600] # 26.4559, 0.7183
    # kernel 3
    # search_range[0.15] = [600] # 26.9277, 0.7343
    # kernel 4
    # search_range[0.15] = [600] # 26.2285, 0.7075
    # kernel 5
    # search_range[0.15] = [600] # 27.7805, 0.7707
    # kernel 6
    # search_range[0.15] = [600] # 27.5230, 0.7630
    # kernel 7
    # search_range[0.15] = [600] # 26.9626, 0.7404
    # kernel 8
    # search_range[0.15] = [600] # 26.5877, 0.7254


    kernel_fp = 'kernels/levin_8.png'
    ## peak =  50, CBSD68_cut, poisson noise removal task. Coco with r=0.25, t=0.33
    # kernel 1
    search_range[0.16] = [375] # 25.7380, 0.6906   
    # kernel 2
    # search_range[0.16] = [375] # 25.5676, 0.6817
    # kernel 3
    # search_range[0.16] = [400] # 26.1639, 0.7072
    # kernel 4
    # search_range[0.16] = [375] # 25.3189, 0.6701
    # kernel 5
    # search_range[0.16] = [375] # 26.8882, 0.7412 
    # kernel 6
    # search_range[0.16] = [375] # 26.5183, 0.7289
    # kernel 7
    # search_range[0.16] = [375] # 26.0929, 0.7098
    # kernel 8
    # search_range[0.16] = [375] # 25.6744, 0.6911
    ...
    search_level = [0.16]# [5, 10, 15, 20, 25, 30]
...
# plot_psnr(level, lambda, peak, 0)

# max_psnr, max_level, max_lamb = search_args()


# 0001.png, kernel1
# plot_psnr(0.15, 570, 100, 0) # 25.56 

# 0005.png, kernel8
plot_psnr(0.18, 500, 50, 0) # 25.33
# plot_psnr(0.16, 400, 50, 0) # 25.33
# plot_psnr(0.14, 300, 50, 0) # 25.30
# plot_psnr(0.12, 240, 50, 0) # 25.23


# butterfly, kernel3

# plot_psnr(0.08, 120, 50, 0) # 25.58
# plot_psnr(0.10, 180, 50, 0) # 25.87
# plot_psnr(0.12, 220, 50, 0) # 25.96
# plot_psnr(0.14, 280, 50, 0) # 25.98
# plot_psnr(0.16, 360, 50, 0) # 25.96


# butterfly, kernel1
# plot_psnr(0.14, 280, 50, 0) # 25.37


# butterfly, kernel2
# plot_psnr(0.16, 360, 50, 0) # 25.25
# plot_psnr(0.14, 300, 50, 0) # 25.19

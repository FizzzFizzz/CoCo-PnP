...
class PnP_PEGD(nn.Module):
    ...
    def forward(self, initial_uv, f, clean, sigma=25.5, lamb=690, sigma2=1.0, denoisor_sigma=25, irl1_iter_num=10, eps=1e-5,alpha = 0.): 
        ...
        for k in range(self.nb):
            ...
            # print(k)
            vv = u-lamb_+((u-lamb_)**2+4*lamb_*f)**0.5
            vv = vv / 2
            v= 0.8*vv + 0.2*u
...
def search_args():
    sigma = 20
    utils_logger.logger_info('denoise', log_path='log/peak_{}/logger.log'.format(sigma))
    ...
    # dataset_root = './trainsets/DIV2K_valid_HR_cut8_small/'
    ...
    # search_range[0.06] = [40] # 29.9923, 0.8625
    # ## peak = sigma = 20, CBSD68, poisson noise removal task.
    search_range[0.08] = [50] # 28.7672, 0.8332
...
# plot_psnr(denoiser_level, lambda, peak)


# 0029.png, 
plot_psnr(0.08, 45, 20) # 28.84
# plot_psnr(0.06, 40, 30) # 30.04

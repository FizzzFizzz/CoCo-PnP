...
def data_solution(x, FB, FBC, F2B, FBFy, alpha, sf):
    ...
    # if you are using old pytorch.
    x1_real = torch.real(x1)
    x1_imag = torch.imag(x1)
    FBR_real = torch.mean(splits(x1_real, sf), dim=-1, keepdim=False)
    FBR_imag = torch.mean(splits(x1_imag, sf), dim=-1, keepdim=False)
    FBR = FBR_real+FBR_imag*1j

    # FBR = torch.mean(splits(x1, sf), dim=-1, keepdim=False) # only supports new pytorch.
